...
class SubspaceClusterRNN(nn.Module):
    def __init__(self, seqlen, dim, rnn='lstm',  hidden_size=256, out_size=1, bidirectional=True, num_layers=1, drp=0.1, agg='mean', bias=True):
        super(SubspaceClusterRNN, self).__init__()
...
if __name__=="__main__":
    import tqdm
    x = torch.randn((32,100,1343))
    y = torch.randint(0, 20, (32, 100,)).float()
    model = SubspaceClusterRNN(100, 1343, hidden_size=32, out_size=1, rnn="lstm", bidirectional=False)
