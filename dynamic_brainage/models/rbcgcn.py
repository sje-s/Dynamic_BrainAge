...
class rBC_GCN(nn.Module):
    def __init__(self, GPC_dim_1, GPC_dim_2, GPC_dim_3, EP_dim, NP_dim, node_dim=53, seqlen=448, rnn_type="gru", rnn_dim=32):
        self.node_dim = node_dim
        super(rBC_GCN, self).__init__()
        ...
        if rnn_type == "rnn":
            self.rnn = nn.RNN(NP_dim, rnn_dim)
        elif rnn_type == "gru":
            self.rnn = nn.GRU(NP_dim, rnn_dim)
        else:
            self.rnn = nn.LSTM(NP_dim, rnn_dim)

        self.fc = nn.Linear(rnn_dim*seqlen, 1)
        nn.init.constant_(self.fc.bias, 0)
        
    def forward(self, x):
        if len(x.shape) < 4:
            xz = torch.zeros((x.shape[0],x.shape[1],self.node_dim,self.node_dim), device=x.device)
            xz[:,:,*np.triu_indices(self.node_dim,k=1)] = x
            xz = xz.permute([0,1,3,2])
            xz[:,:,*np.triu_indices(self.node_dim,k=1)] = x
            x = xz
        xts = []
        for t in range(x.shape[1]):
            xt = x[:,t,:,:].view(x.shape[0],1,x.shape[2],x.shape[3])
            xt = self.GPC_1(xt)
            xt = F.relu(xt)
            ...
            xt = self.NP(xt)
            xt = F.relu(xt)
            xts.append(xt.squeeze())
        xo = torch.stack(xts, 1)
        xo,_ = self.rnn(xo)
        xo = xo.flatten(1)
        yh = self.fc(xo)
