...
class ConvertData(Dataset):
    def __init__(self,
                ...
                #  window_size=40,
                 ...
                 age_csv="/data/users3/bbaker/projects/Dynamic_BrainAge/data/HCP_HCPA_UKB_age_filepaths_dFNC_sMRI_cogScores_v2.csv",
        ...
        self.N_subjects = min(N_subs, len(self.idxs))
        self.age = np.array(all_data.loc[self.idxs, "age"]).reshape(
            self.N_subjects, 1).astype('float32')
    ...
    def __getitem__(self, k):
        ...
        dfnc = data['tcraw'].astype('float32')
...
if __name__ == "__main__":
    ...
    for batch_i, (fnc, fname, age,session) in pbar:
        ...
        cfname = "/data/users3/bbaker/projects/Dynamic_BrainAge/data/tcraw_v2/" + fname + ".pt"
        torch.save(fnc[0], cfname)
        rows.append(dict(old_filename=fname[0], new_filename=cfname, age=age, session=session))
        pbar.update(1)
        #break
    df = pd.DataFrame(rows)
    df.to_csv('./data/tcraw_converted_files_v2.csv')
