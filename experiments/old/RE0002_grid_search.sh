#!/bin/bash
experiment_name="RE0002_grid_search"
grid_file="experiments/grids/"$experiment_name
rm $grid_file
criterion_=( "MSELoss" )
...
#model_kwargs_=( '{"hidden_size":64}' '{"hidden_size":128}' '{"hidden_size":256}' '{"hidden_size":64, "bidirectional":0}' '{"hidden_size":128, "bidirectional":0}' '{"hidden_size":256, "bidirectional":0}' )
model_kwargs_hidden_size_=( '"hidden_size":64' '"hidden_size":128' '"hidden_size":256' )
model_kwargs_bidirectional_=( '"bidirectional":true' '"bidirectional":false' )
model_kwargs_drp_=( '"drp":0.1' '"drp":0.2' '"drp":0.3' )
...
for model_kwargs_hidden_size in "${model_kwargs_hidden_size_[@]}"; do 
for model_kwargs_bidirectional in "${model_kwargs_bidirectional_[@]}"; do 
for model_kwargs_hidden_drp in "${model_kwargs_drp_[@]}"; do 
model_kwargs="{"$model_kwargs_hidden_size","$model_kwargs_bidirectional","$model_kwargs_hidden_drp"}"
...
sbatch --array=1-${run}%10 -J $experiment_name -e "slurm/logs/"$experiment_name"-%a.err" -o "slurm/logs/"$experiment_name"-%a.out" slurm/gpu_runner_grid.sh $grid_file
