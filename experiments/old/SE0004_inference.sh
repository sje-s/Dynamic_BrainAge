...
model_kwargs_=( '{"num_layers":3,"rnn":"gru"}' )
...
sbatch -J "inference-"$experiment_name"-"$run -e "slurm/logs/inference_"$experiment_name"-"$run".err" -o "slurm/logs/inference_"$experiment_name"-"$run".out" slurm/gpu_runner_seq.sh $args
run=$((run+1))
done
done
done
