#!/bin/bash
experiment_name="SE0005_lsgru_long"
criterion_=( "MSELoss" )
test_dataset_=( "ukbhcp1200" )
test_dataset_args_=( "[]" )
test_dataset_kwargs_=( '{"N_subs":10000,"sequential":true}' )
train_dataset_=( "None" )
train_dataset_args_=( "[]" )
train_dataset_kwargs_=( "{}" )
model_=( "simpleseqrnn" )
model_args_=( "[448,1378]" )
model_kwargs_=( '{"num_layers":3,"rnn":"gru"}' '{"num_layers":3,"rnn":"lstm"}' '{"num_layers":3,"rnn":"rnn"}')
inference_model_=( "best_full.pth" )
optimizer_=( "Adam" )
optim_kwargs_=( "{}" )
batch_size_=( "64" )
lr_=( "1e-3" )
...
inference_model_x=$logdir"/checkpoints/"$inference_model
args=""
...
args=$args"--inference-model "$inference_model_x" "
...
sbatch -J "inference-"$experiment_name"-"$run -e "slurm/logs/"$experiment_name"-"$run".err" -o "slurm/logs/"$experiment_name"-"$run".out" slurm/gpu_runner_seq.sh $args
run=$((run+1))
done
done
done
