...
train_dataset_args_=( "[]" )
train_dataset_kwargs_=( '{"N_subs":10000,"sequential":true}' )
test_dataset_=( "valid" )
test_dataset_args_=( "[]" )
...
sbatch -J $experiment_name"-"$run -e "slurm/logs/"$experiment_name"-"$run".err" -o "slurm/logs/"$experiment_name"-"$run".out" slurm/gpu_runner_seq.sh $args
run=$((run+1))
done
done
done
