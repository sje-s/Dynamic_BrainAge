#!/bin/bash
experiment_name="A003_feng_smri"
criterion_=( "L1Loss" )
train_dataset_=( "ukbhcp_v2_smri" )
train_dataset_args_=( "[]" )
train_dataset_kwargs_=( '{"N_subs":17522,"N_timepoints":-1,"sequential":0}' )
test_dataset_=( "valid" )
test_dataset_args_=( "[]" )
test_dataset_kwargs_=( "{}" )
model_=( "conv3d" )
model_args_=( "[[1,1,121,145,121],1]" )
model_kwargs_=( '{"hidden_neurons":[4,8,16,32,64],"hidden_neurons_linear":[],"maxpool_after":1}' )
...
sbatch -J $experiment_name"-"$run -e "slurm/logs/"$experiment_name"-"$run".err" -o "slurm/logs/"$experiment_name"-"$run".out" slurm/gpu_runner_a100.sh $args
run=$((run+1))
done
done
done
